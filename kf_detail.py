...
class EKF_UKF():
    def EKF_UKF_Thev(self, SoC_upd_init, current):
        ...
        States_real = np.array([SoC_real[1, 1], 0])  # (SoC_real, Up_real)
        States_upd = np.array([SoC_upd_init, 0])  # (SOC_upd, Up_upd)
        SoC_AH[1, 1] = SoC_upd_init  # Initail value of AH
        SoC_EKF[1, 1] = SoC_upd_init
        # SoC_UKF[:, 1] = SoC_upd_init
        # Errors
        Err_EKF = np.zeros((1, N))
        # Err_UKF = np.zeros((1, N))
        Err_AH = np.zeros((1, N))
        ...
        lam = 1.5
       # weight
        Wm =[lam / (n + lam ), 0.5 / (n + lam ) + np.zeros(1, 2 * n)]
        Wc = Wm
        ...
        for T in range(2, N):
            ...
            for t in range((T-1) * ts / tr - (ts / tr - 2), (T - 1) * ts / tr + 1):
                ...
                A2 = np.exp(-tr / tao)
                A = np.array([[1, 0], [0, A2]])  # State transformation matrix
                B1 = - tr / (Capacity * 3600)
                B2 = Rp * (1 - np.exp(-tr / tao))
                B = np.array([B1, B2])  # Input control matrix

                States_real[:, t] = A * States_real[:, t - 1] + B * I_real[1, t] + [np.sqrt(Qs) * np.random.randn(), np.sqrt(Qu) * np.random.randn()]
            ...
            UL_ob_EKF = UOC_real - States_real[2, t] - I_real[1, t] * Rint_real + np.sqrt(R) * np.random.randn()
            # UL_ob_UKF = UOC_real - B2 * I_real[1, t] - I_real[1, t] * Rint_real + np.sqrt(R) * np.random.randn()
            I_ob = I_real(t) + (0.01 * Capacity) * np.random.randn()  # 观测误差过大�? KF 效果甚至低于 AH
            ...
            tao = Rp * Cp
            A = np.array([[1, 0], [0, np.exp(-1*ts / tao)]])  # State transformation matrix
            B = np.array([-1*ts / (Capacity * 3600), Rp * (1 - np.exp(-1*ts / tao))])  # Input control matrix
            ...
            P_Cov = np.dot(np.dot(A, P_Cov), A.T) + np.array([[Qs,0],[0, Qu]])

            # UOC和SOC的关系式，用poly7阶进行拟合
            ...
            C = [C1 - 1]
            # update
            K = np.dot(np.dot(P_Cov, C.T), np.linalg.inv(np.dot(np.dot(C, P_Cov), C.T)+R)) # kalman gain matrix
            States_upd[:, T] = States_pre + np.dot(K, (UL_ob_EKF - UL_pre))
            ...
            for i in range(1, 2 * n + 1):
                ...
                tao = Rp * Cp
                gamma(i) = UOC_pre - I_ob * Ro_pre - I_ob * Rp * (1 - np.exp(-1*ts / tao))
        ...
        avr_err_EKF = np.mean(Err_EKF)
        std_err_EKF = np.std(Err_EKF, 0)
        return avr_err_EKF, std_err_EKF, SoC_real, SoC_AH, SoC_EKF, Err_EKF, Err_AH
        # avr_err_UKF = np.mean(Err_UKF)
        # std_err_UKF = np.std(Err_UKF, 0)

        # Display ------------------------------------------------------
    def display(self, SoC_upd_init, current):
        N = 5000
        ts = 1
        tr = 0.1
        avr_err_EKF, std_err_EKF, SoC_real, SoC_AH, SoC_EKF, Err_EKF, Err_AH = self.EKF_UKF_Thev(SoC_upd_init, current)
        T = [_ for _ in range(1, N)]
        plt.figure(figsize=(10, 8))

        plt.subplot(2, 1, 1)
        plt.plot(T, SoC_real[1, 1: (ts / tr):(N * ts / tr - 1)], 'LineWidth', 2)
        plt.plot(T, SoC_AH[1, 1: N], '-.m', T, SoC_EKF[1, 1: N], '-.g')
        # plt.plot(T, SoC_UKF(1: N), '-.', 'Color', [1 0.5 0])
        plt.xlabel('t(s)')
        plt.ylabel('SOC')
        # plt.legend('SoC_{Real}', 'SoC_{AH}', 'SoC_{EKF}', 'SoC_{UKF}')
        plt.legend('SoC_{Real}', 'SoC_{AH}', 'SoC_{EKF}')
        plt.subplot(2, 1, 2)
        plt.plot(T, Err_AH[1, 1: N], '-r', T, Err_EKF[1, 1: N], '-.g')
        # plt.plot(T, Err_UKF(1, 1: N), '-.', 'Color', [1 0.5 0])
        plt.xlabel('t(s)')
        plt.ylabel('error')
        # plt.legend('Err_{AH}', 'Err_{EKF}', 'Err_{UKF}', 'Location', 'Best')
        plt.legend('Err_{AH}', 'Err_{EKF}', 'Location', 'Best')